...
Bind facility browsing before GPU startup. A failed WebGL context opens a clearly labelled schematic campus map with the same 35 selectable facilities. Never label this a 3D tour. Keep the directory, source areas and floor programs available. Add a separate floor-plan modal for all browsers, with the current proposed room proportions, two support strips, 12-foot branching circulation and six accessible room buttons. The plan can be opened alongside the optional 3D tour on capable devices. On narrow screens the plan stacks below its header and above the room list; modal focus returns to its initiating action.
...
## September 17 physical materials and living detail
Default to natural daylight with analytic atmosphere and outdoor reflections. Dusk remains available with restrained luminaire intensity; glass, foliage, water and masonry do not glow. Model foliage as branches and folded leaves, with accurate planting setbacks. Keep roadway emission within narrow embedded guidance strips over asphalt. Fine Blender furniture appears within 180m of the camera. Full fleet population must be identical under automation and normal browsing. Offline geometry renders cannot approve GPU appearance or physical-device performance.
