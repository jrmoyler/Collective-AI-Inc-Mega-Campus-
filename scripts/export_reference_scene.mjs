...
const furnitureFile=new URL('../public/models/campus-street-furniture.glb',import.meta.url);
if(fs.existsSync(furnitureFile)){
 const bytes=fs.readFileSync(furnitureFile);
 const asset=await new GLTFLoader().parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');
 scene.add(createStreetFurniture(asset.scene));
}
...
// Live rig references are animation state, not glTF extras. Serializing them
// recursively duplicates mesh buffers and can exceed V8's JSON string limit.
scene.traverse(o=>{o.userData=Number.isFinite(o.userData.facility)?{facility:o.userData.facility}:{};});
