...
let vertices=0,meshes=0;scene.traverse(o=>{if(o.isMesh){meshes++;const a=o.geometry.attributes.position;vertices+=a.count*(o.count||1);for(const n of a.array)if(!Number.isFinite(n))throw new Error('Non-finite geometry '+o.name);if(o.material.map){if(o.geometry.type==='PlaneGeometry'&&o.geometry.parameters.width===2600)o.material.color.set(0x42513b);o.material.map=null;} }});fs.mkdirSync('evidence',{recursive:true});fs.writeFileSync('evidence/geometry-check.json',JSON.stringify({meshes,vertices,facilities:FACILITIES.length,treeCount:landscape.treeCount,finitePositions:true,renderer:'Three.js scene graph export, not WebGL'},null,2));
