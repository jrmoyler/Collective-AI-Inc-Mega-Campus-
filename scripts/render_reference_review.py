...
for name,position in [('aerial',(90,-1100,960)),('north',(-100,1000,800))]:
 cam.location=position;cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(Path(__file__).resolve().parents[1]/('evidence/offline-'+name+'.jpg'));bpy.ops.render.render(write_still=True)
