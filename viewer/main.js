...
function setDay(day){
 ...
 try{
  ...
  const next=pmrem.fromScene(envScene,.04,.1,10000);scene.environment=next.texture;
...
async function boot(){
 ...
 new GLTFLoader().load(`${import.meta.env.BASE_URL}models/campus-street-furniture.glb`,g=>{streetFurniture=createStreetFurniture(g.scene);streetFurniture.userData.update(camera.position);scene.add(streetFurniture);},undefined,e=>console.warn('Street furniture detail unavailable',e));
