...
export function createPerformanceReport(){
 ...
 const segments=Object.entries(groups).map(([view,rows])=>{const a=rows.map(r=>r.frameMs).sort((a,b)=>a-b),mean=a.reduce((a,b)=>a+b,0)/a.length;return {view,samples:a.length,meanFrameMs:+mean.toFixed(2),p95FrameMs:+a[Math.min(a.length-1,Math.floor(a.length*.95))].toFixed(2),meanFPS:+(1000/mean).toFixed(1),renderSize:[rows.at(-1).width,rows.at(-1).height],meshes:rows.at(-1).meshes,drawCalls:rows.at(-1).drawCalls,triangles:rows.at(-1).triangles??null};});
 const result={schema:1,capturedAt:new Date().toISOString(),durationSeconds:(Date.now()-started)/1000,userAgent:navigator.userAgent,viewport:[innerWidth,innerHeight],devicePixelRatio,segments,notes:['Real frame intervals from this browser, not a GPU timer.','Exterior draw calls and triangles include shadow and postprocessing passes; mesh count is visible scene-graph mesh objects.','User agent does not certify a physical device; record model and conditions separately.','A device performance report is not reference-image acceptance.']};
