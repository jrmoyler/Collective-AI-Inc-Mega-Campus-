...
export function createInterior(canvas,f,{onRoom,onFloor,reduced=false,engineOverride=null,quality='balanced',onFrame=()=>{}}){
 ...
 function build(l){
  ...
  lights=layout.rooms.map((r,i)=>{const lamp=new SpotLight('room luminaire '+i,new Vector3(r.x,3.58,r.z),new Vector3(0,-1,0),2.6,1,scene);lamp.intensity=30;lamp.range=25;lamp.diffuse=new Color3(1,.88,.68);lamp.includedOnlyMeshes=scene.meshes.filter(m=>m.name.startsWith('room-'+(i+1)+':')||m.name.startsWith('architecture'));return lamp;});
