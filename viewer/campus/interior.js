...
export function createInterior(canvas,f,{onRoom,onFloor,reduced=false,engineOverride=null,quality='balanced',onFrame=()=>{},onLifecycle=()=>{},onError=error=>console.error(error)}){
 ...
 function build(l){
  ...
  lights=layout.rooms.map((r,i)=>{const lamp=new SpotLight('room luminaire '+i,new Vector3(r.x,3.58,r.z),new Vector3(0,-1,0),2.6,1,scene);const kind=assembly.root.children[i].userData.kind;const technical=['lab','clinical','servers','industrial','electrical','process','fabrication','maintenance'].includes(kind);lamp.intensity=18+Math.min(24,r.w*r.d*.08);lamp.range=Math.hypot(r.w,r.d)+4;lamp.diffuse=technical?new Color3(.89,.95,1):new Color3(1,.91,.78);lamp.includedOnlyMeshes=scene.meshes.filter(m=>m.name.startsWith('room-'+(i+1)+':')||m.name.startsWith('architecture'));return lamp;});
