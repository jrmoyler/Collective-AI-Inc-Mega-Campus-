...
export function createInterior(canvas,f,{onRoom,onFloor,reduced=false,engineOverride=null,quality='balanced',onFrame=()=>{},onLifecycle=()=>{},onError=error=>console.error(error)}){
 ...
 let scene,camera,layout,level=0,flight=null,currentRoom=null,disposed=false,shadow=null,shadowLight=null,lights=[];const held=new Set();let contextLost=false,awaitingFrame='floor-first-frame';
 ...
 function build(l){
  ...
  try{
  flight?.pause();flight=null;held.clear();shadow?.dispose();shadow=null;scene?.dispose();level=l;currentRoom=null;layout=assembly.layout;scene=new Scene(engine);scene.useRightHandedSystem=true;scene.clearColor=new Color4(.27,.35,.40,1);scene.collisionsEnabled=true;
  scene.imageProcessingConfiguration.toneMappingEnabled=true;scene.imageProcessingConfiguration.exposure=1.06;scene.imageProcessingConfiguration.contrast=1.18;
  ...
  if(!engineOverride)scene.environmentTexture=new RawCubeTexture(scene,faces,16,Engine.TEXTUREFORMAT_RGBA,Engine.TEXTURETYPE_UNSIGNED_BYTE,true);
  const finishes=createInteriorMaterials(scene,new Set(assembly.root.children.flatMap(g=>g.children.map(m=>m.material.name))));for(const m of Object.values(finishes))m.maxSimultaneousLights=8;
  ...
  const hemi=new HemisphericLight('daylight fill',new Vector3(0,1,0),scene);hemi.intensity=.38;hemi.groundColor=new Color3(.18,.17,.15);
  const sun=new DirectionalLight('terrace daylight',new Vector3(.45,-.8,.6),scene);sun.intensity=.92;sun.diffuse=new Color3(1,.92,.79);
  lights=layout.rooms.map((r,i)=>{const lamp=new SpotLight('room luminaire '+i,new Vector3(r.x,r.height-.32,r.z),new Vector3(0,-1,0),1.45,1.8,scene);const kind=assembly.root.children.find(g=>g.name.startsWith('room-'+(i+1)+':')).userData.kind;const technical=r.fitout?.technical||['lab','clinical','servers','industrial','electrical','process','fabrication','maintenance','stage','shielded','aerospace','logistics','algae','test-track'].includes(kind);lamp.intensity=(14+Math.min(19,r.w*r.d*.065))*((r.height-.32)/3.58)**2;lamp.range=Math.max(Math.hypot(r.w,r.d)+4,r.height+2);lamp.diffuse=technical?new Color3(.89,.95,1):new Color3(1,.91,.78);lamp.includedOnlyMeshes=scene.meshes.filter(m=>m.name.startsWith('room-'+(i+1)+':')||m.name.startsWith('occupants room '+(i+1)+'/')||m.name.startsWith('architecture'));return lamp;});
